...
public final class SpanUtils {
     ...
     * 替换文字
     *
     * @param needRepeat 需要替换的原始文字
     * @param repeat     将要替换成的文字
     * @return
     */
    public SpanUtils replace(CharSequence needRepeat, CharSequence repeat) {
        append("");
        SerializableSpannableStringBuilder builder = new SerializableSpannableStringBuilder();
        builder = mBuilder;
        String s = builder.toString();

        int i = s.indexOf(needRepeat.toString());

        mBuilder.replace(i, i + needRepeat.length(), repeat);
...
//            }
    ...
    static class CustomImageSpan extends CustomDynamicDrawableSpan {
        ...
        private CustomImageSpan(final Bitmap b, final int verticalAlignment) {
            ...
            mDrawable = new BitmapDrawable(GlobalUtils.getApp().getResources(), b);
            mDrawable.setBounds(
        ...
        public Drawable getDrawable() {
            ...
            if (mDrawable != null) {
                ...
                try {
                    InputStream is =
                            GlobalUtils.getApp().getContentResolver().openInputStream(mContentUri);
                    bitmap = BitmapFactory.decodeStream(is);
                    drawable = new BitmapDrawable(GlobalUtils.getApp().getResources(), bitmap);
                    drawable.setBounds(
                ...
                try {
                    drawable = ContextCompat.getDrawable(GlobalUtils.getApp(), mResourceId);
                    drawable.setBounds(
                            0, 0, drawable.getIntrinsicWidth(), drawable.getIntrinsicHeight()
                    );
                } catch (Exception e) {
