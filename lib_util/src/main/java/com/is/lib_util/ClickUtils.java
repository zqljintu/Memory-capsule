...
public class ClickUtils {
    ...
    /**
     * 最后点击时间 响应全局所有调用{@link #applySingle}
     */
    ...
    /**
     * 普通防抖动快速点击事件处理
     * @param duration 点击处理间隔时间
     */
    public static boolean applySingle(@IntRange(from = 0) long duration) {
        long nowClickTime = System.currentTimeMillis();
        if ((nowClickTime - LAST_CLICK_TIME) >= duration) {
            LAST_CLICK_TIME = nowClickTime;
            return true;
        }
        return false;
    }
